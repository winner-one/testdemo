    ...
    var Controller = {
        ...
        profile: function () {
            let formatterHtml = {
                ...
                shareUser: (row, value) => {
                    if (row.user) {
                        return `<img style="width:24px;height:24px;margin-right:10px" src="${row.user.avatar}" /><div>${row.user.nickname}</div>`
                    }
                },
                ...
                shareMessage: (row, value) => {
                    ...
                    if (row.type == "groupon") {
                        ...
                        if (row.goods) {
                            str = `<img style="width:24px;height:24px;margin-right:14px;border:1px solid #E6E6E6" src="${row.goods.image}" /><div class="flex-1 ellipsis-item">${row.goods.title}</div>`
                        }
                    }
                    return str
                },
                goods: (row, value) => {
                    if (row.goods) {
                        return `<img style="width:24px;height:24px;margin-right:10px" src="${row.goods.image}" /><div class="flex-1 ellipsis-item" style="text-align:left">${row.goods.title}</div>`
                    }
                },
                coupon: (row, value) => {
                    if (row.coupons) {
                    ...
                    return {
                        ...
                        columns: {
                            'money_log': [{
                                ...
                                title: '变动余额',
                                width: '120px',
                                formatter: formatterHtml.changeNumber,
