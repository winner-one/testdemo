define(['jquery', 'bootstrap', 'backend', 'table', 'form', 'toastr'], function ($, undefined, Backend, Table, Form, Toastr) {
    ...
    var Controller = {
                    ...
                    goodsOpt(type, id) {
                        ...
                        switch (type) {
                            ...
                            case 'edit':
                                ...
                                break;
                            case 'yuyue':
                                Fast.api.open('shopro/goods/days_price/dashboard/ids/' + id + "?id=" + id + "&type=edit", '商品统计', {
                                    area: ["100%", "100%"],
                                    callback() {
                                        // that.getData();
                                    }
        ...
        add: function () {
            Controller.initAddEdit(null, null, [], [], 'add');
        },
        edit: function () {
            let id, type
            ...
            Controller.initAddEdit(id, type, Config.skuList, Config.skuPrice, 'edit');
        },
        select: function () {
            function debounce(handle, delay) {
                let time = null;
            ...
            var goodsSelect = new Vue({
                ...
                mounted() {
                    if (new URLSearchParams(location.search).get('ids') && this.multiple == 'true') {
                        this.selectedIdsArr = new URLSearchParams(location.search).get('ids').split(',');
                        let numberId = [];
                        this.selectedIdsArr.forEach(i => {
                            numberId.push(Number(i))
                        })
                        this.selectedIdsArr = numberId;
                    }
                    console.log(this.selectedIdsArr)
                    this.goodsData = Config.category
                    this.getList();
                },
                methods: {
                    checkedAll(val) {
                        ...
                        if (val) {
                            this.goodsList.forEach(i => {
                                if (!this.selectedIdsArr.includes(i.id)) {
                                    this.selectedIdsArr.push(i.id)
                                }
                            })
                        } else {
                            this.goodsList.forEach(i => {
                                if (this.selectedIdsArr.includes(i.id)) {
                                    this.selectedIdsArr.splice(this.selectedIdsArr.indexOf(i.id), 1)
                                }
                            })
                        }
                        console.log(this.selectedIdsArr)
                    ...
                    getList(id) {
                        let that = this;
                        let url = "shopro/goods/goods/select";
                        if (id) {
                            url = 'shopro/goods/goods/select?category_id=' + id
                        }
                        Fast.api.ajax({
                            url: url,
                            data: {
                            ...
                            if (that.goodsList.length) {
                                that.goodsList.forEach(i => {
                                    if (!that.selectedIdsArr.includes(i.id)) {
                                        that.isAll = false;
                                    }
                                })
                            }
                    ...
                    selectGoods(item, index) {
                        if (this.multiple == 'true') {
                            if (this.selectedIdsArr.includes(item.id)) {
                                this.selectedIdsArr.splice(this.selectedIdsArr.indexOf(item.id), 1)
                            } else {
                    ...
                    operation(type, row) {
                        ...
                        switch (type) {
                            case 'define':
                                if (this.multiple == 'true') {
                                    Fast.api.ajax({
                                        url: domain + '/addons/shopro/goods/lists?goods_ids=' + that.selectedIdsArr.join(',') + "&per_page=999999999",
                                        loading: false,
                                    }, function (ret, res) {
                                        Fast.api.close({
                                            data: res.data.data,
                                        ...
                                        // that.templateData[that.centerSelect].content.ids = data.data.join(',')
                    ...
                    showLeft(p, c, a, s) {
                        if (p != null && a === null && c === null && s === null) {
                            this.goodsData[p].show = !this.goodsData[p].show;
                        }
                        if (p != null && c != null && a == null && s === null) {
                            this.goodsData[p].children[c].show = !this.goodsData[p].children[c].show;
                        }
                        if (p != null && c != null && a != null && s === null) {
                            this.goodsData[p].children[c].children[a].show = !this.goodsData[p].children[c].children[a].show;
                        }
                        this.$forceUpdate();
                    },
                    selectCategoryLeft(p, c, a, s) {
                        ...
                        if (p != null && a === null && c === null && s === null) {
                            this.goodsData[p].selected = !this.goodsData[p].selected;
                            category_id = this.goodsData[p].id
                        }
                        if (p != null && c != null && a == null && s === null) {
                            this.goodsData[p].children[c].selected = !this.goodsData[p].children[c].selected;
                            category_id = this.goodsData[p].children[c].id
                        }
                        if (p != null && c != null && a != null && s === null) {
                            this.goodsData[p].children[c].children[a].selected = !this.goodsData[p].children[c].children[a].selected;
                            category_id = this.goodsData[p].children[c].children[a].id
                        }
                        if (p != null && c != null && a != null && s != null) {
                            this.goodsData[p].children[c].children[a].children[s].selected = !this.goodsData[p].children[c].children[a].children[s].selected;
                            category_id = this.goodsData[p].children[c].children[a].children[s].id
                        }
        ...
        initAddEdit: function (id, type, skuList, skuPrice, Type) {
                ...
                methods: {
                    ...
                    getEditData() {
                        ...
                        Fast.api.ajax({
                            ...
                            that.goodsDetail.is_ticket = res.data.detail.is_ticket;
                            that.goodsDetail.days = res.data.detail.days;
                            console.log('that.goodsDetail.is_ticket')
                            console.log(that.goodsDetail.is_ticket)
                            ...
                            for (key in that.goodsDetail) {
                                if (res.data.detail[key]) {
                                    ...
                                    that.goodsDetail.store_id = res.data.detail.store_id ? res.data.detail.store_id : '';
