define(['jquery', 'bootstrap', 'backend', 'table', 'form', 'toastr'], function ($, undefined, Backend, Table, Form, Toastr) {
    ...
    var Controller = {
        ...
        select1: function () {
            ...
            Table.api.init({
                extend: {
                    index_url: 'shopro/store/store/select1',
                }
            });
                        ...
                        // { field: 'images', title: __('Images') },
                        {
                            field: 'operate', title: __('Operate'), events: {
                                'click .btn-chooseone': function (e, value, row, index) {
                                    ...
                                    let data = []
                                    data.list = [row]
                                    data.ids = row.id.toString()
                                    Fast.api.close({ data: data, multiple: multiple });
                                },
                            }, formatter: function () {
            ...
            $(document).on("click", ".btn-choose-multi", function () {
                ...
                multiple = multiple == 'true' ? true : false;
                let row = []
                row.list = table.bootstrapTable("getAllSelections")
            ...
            var storeDetail = new Vue({
                ...
                mounted() {
                    ...
                    if (this.opttype == 'add') {
                        ...
                        this.lnglat = [];
                        this.lnglat.push(Config.row.longitude);
                        this.lnglat.push(Config.row.latitude)
                ...
                methods: {
                    ...
                    addCircle() {
                        ...
                        that.circleEditor.on('move', function (event) {
                            that.lnglat = []
                            that.lnglat.push(that.circle.getCenter().lng)
                            that.lnglat.push(that.circle.getCenter().lat)
                        })
                    },
                    addMarker() {
                        let that = this;
                        ...
                        that.map.on('click', function (event) {
                            that.lnglat = []
                            that.lnglat.push(event.lnglat.lng)
                            that.lnglat.push(event.lnglat.lat)
                            if (that.storeForm.service_type == 'radius') {
                                that.circle.setCenter(that.lnglat)
                                that.map.setFitView([that.circle])
                            } else {
                    ...
                    areaOptionsChange(val) {
                        let that = this;
                        let address = val.join(',') + that.storeForm.address;
                        this.getlnglat(address)
                    },
                    radioChange(val) {
                        if (val == 'area') {
                            this.circle.hide()
                    ...
                    addImg1(type, index, multiple) {
                        let that = this;
                        parent.Fast.api.open("general/attachment/select?multiple=" + multiple, "选择图片", {
                            callback: function (data) {
                                that.storeForm.logo_image = data.url;
                            }
                        });
                        return false;
                    },
                ...
                watch: {
                    storeForm: {
                        handler: function (oval, nval) {
                            let that = this;
                            that.circle.setRadius(oval.service_radius);
                            if (!Config.hasAmap) {
                                // Toastr.error('请在商城配置->第三方服务中配置高德地图');
                                return false;
                            }
