...
trait OrderOperSendGet
{
    ...
    {
        $is_ticket =0;
        // $is_ticket = Goods::where('id', $order->item[0]->goods_id)->value('is_ticket');
        $code = $is_ticket != '0' ? Verify::getCode() : 0;          
            ...
            switch ($item['dispatch_type']) {
                case 'selfetch':
                    if ($item['goods_type'] == 'virtual') {
                        // 虚拟商品，核销券，自动发货
                        $this->selfetchSendItem($order, $item, ['oper_type' => 'system'], $is_ticket, $code);
                    }
                    break;
                case 'autosend':
                    // 自动发货
     ...
     */
    private function selfetchSendItem($order, $item, $data = [], $is_ticket = 0, $code = 0)
    {
        // 获取配送模板
        $dispatch = Dispatch::where('type', $item['dispatch_type'])->where('id', $item['dispatch_id'])->find();
    ...
    public function sendItem($order, $item, $data = [])
    {
        ...
        if (isset($data['ext']) && $data['ext']) {
            $item->ext = json_encode($item->setExt($item, $data['ext']));
        }
     ...
     * 生成码
     */
    private function createVerify($order, $item = null, $data = [], $code = 0)
    {
        $verify = new Verify();
        Log::write('code' . $code);
        ...
        $i = 0;
        do {
            // 循环生成唯一核销码（数据表code索引已关闭）
            $is_error = false;
            try {
                if ($code) {    //订房及门票是否生成同一张核销卷
                    $verify->code = $code;
                } else {
     ...
     */
    private function getOper($oper_type, $oper = null)
    {
        $oper = null;
        $oper_iden = '系统自动';
        if ($oper_type == 'system') {
