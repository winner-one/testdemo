...
class Confirm
{
    ...
    public function orderConfirmAfter(&$params)
    {
        ...
        // 判断订单是否全部确认收货，如果有子订单进入售后不算，佣金也将失效
        $noConfirm = OrderItem::where('order_id', $order['id'])->where('dispatch_status', '<>', OrderItem::DISPATCH_STATUS_GETED)->where('aftersale_status', '0')->count();

        // 订单已全部确认收货
        if ($noConfirm <= 0) {
            // 触发订单完成钩子，可以在这里处理分销佣金等
            $data = ['order' => $order];
            \think\Hook::listen('order_finish', $data);
        }
