...
class Auth extends \fast\Auth
{
    ...
    public function login($username, $password, $keeptime = 0)
    {
        ...
        $group_id = \think\Db::name('auth_group_access')->where('uid', $admin_data['id'])->value('group_id');
        if ($group_id != '1' && $group_id != '2') {        //判断不为超级管理员
            if (!$admin_data['store_id']) {         //如果为三级管理员，没有设置门店信息则不让获取后台信息
                $admin_data['store_id'] = 999999;
            }
        } else {
            $admin_data['store_id'] = 0;
        }
