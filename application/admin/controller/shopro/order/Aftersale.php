...
class Aftersale extends Backend
{
    ...
    private function buildSearchOrder()
    {
        ...
        if ($this->auth->store_id) {
            $where['store_id'] = $this->auth->store_id;
        }else{
            $where = [];
        }
        $orders = $orders->where($where)->whereExists(function ($query) use ($search, $status) {
     ...
     * 组合搜索 废弃
     */
    private function buildSearchOrderOld()
    {
        $search = $this->request->get("search", '');        // 关键字
        $status = $this->request->get("status", 'all');
        ...
        $aftersales = $this->model->withTrashed();
        ...
        if ($search) {
            ...
            unset($v);
            $aftersales = $aftersales->where(function ($query) use ($searcharr, $search, $tableName) {
                $query->where(implode("|", $searcharr), "LIKE", "%{$search}%")
